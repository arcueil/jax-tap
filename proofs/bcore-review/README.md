...
| A2 | `verbose(jax.checkpoint(f))` emits ZERO events (silent) | **CONFIRMED real gap, cheaply fixable.** `remat2` prim carries `params['jaxpr']` (the inner scan); walker binds it opaquely instead of recursing. Fix: add `remat2` to the recursion set (like `jit`). Scheduled for consolidated remediation. |
